...
const
  LibraryName = 'tensorflowlite.so';
  LibraryNameGPU = 'tensorflowlite_gpu.so';
{$ENDIF ANDROID}
{$IFDEF MSWINDOWS}
...
type
  ...
  public
    ...
    function ResizeInputTensor(Interpreter: Pointer; InputIndex: Int32; InputSize: Int32): TFLiteStatus;
...
type
  ...
  TFLiteInterpreterResizeInputTensor = function(Interpreter: Pointer; input_index: Int32; const input_dims: Pointer; input_dims_size: Int32): TFLiteStatus; stdcall;
  // Mobile GPU
...
var
  ...
  InterpreterResizeInputTensor: TFLiteInterpreterResizeInputTensor = nil;
...
{$IFDEF ANDROID}
  FPath := System.IOUtils.TPath.Combine(System.IOUtils.TPath.GetHomePath, LibraryName);
  LibraryModule := System.SysUtils.LoadLibrary(PWideChar(FPath));

  FPath := System.IOUtils.TPath.Combine(System.IOUtils.TPath.GetHomePath, LibraryNameGPU);
...
{$ENDIF ANDROID}
  ...
  InterpreterResizeInputTensor := GetProcAddress(LibraryModule, 'TfLiteInterpreterResizeInputTensor');
  ...
  if (@ModelCreateFromFile = nil) or (@LiteVersion = nil) or (@InterpreterOptionsCreate = nil) or
    ...
    (@TensorNumDims = nil) or (@TensorName = nil) or (@InterpreterResizeInputTensor = nil) or
...
function TTensorFlowLiteFMX.ResizeInputTensor(Interpreter: Pointer; InputIndex: Int32; InputSize: Int32): TFLiteStatus;
begin
  Result := InterpreterResizeInputTensor(Interpreter, InputIndex, nil, InputSize);
end;
